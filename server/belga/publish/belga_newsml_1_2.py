...
class BelgaNewsML12Formatter(NewsML12Formatter):
    ...
    SD_CP_NAME_ROLE_MAP = {"belga_text": "Belga text"}
    NEWSCOMPONENT2_ROLES: NewsComponent2Roles = NewsComponent2Roles(
        "URL",
        "Picture",
        "Gallery",
        "Audio",
    ...
    def _format_newsmanagement(self, newsitem):
        ...
        SubElement(news_management, "FirstCreated").text = self._get_formatted_datetime(
            self._current_item.get("firstcreated")
        )
        SubElement(news_management, "ThisRevisionCreated").text = self._string_now
    ...
    def _format_text(self, newscomponent_1_level, item):
        ...
        newscomponent_2_level = SubElement(
            newscomponent_1_level, "NewsComponent", {XML_LANG: item.get("language")}
        )
    ...
    def _format_url(self, newscomponent_1_level, item_url):
        ...
        newscomponent_2_level = SubElement(
            newscomponent_1_level, "NewsComponent", {XML_LANG: item_url.get("language")}
        )
        ...
        for role, key in (("Title", "description"), ("Locator", "url")):
            ...
            SubElement(characteristics, "SizeInBytes").text = (
                str(len(item_url[key])) if item_url.get(key) else "0"
            )
            SubElement(
                characteristics,
                "Property",
    ...
    def _format_picture(self, newscomponent_1_level, picture):
        ...
        image_id = (picture.get("guid") or picture["_id"]) + picture.get(
            "language", "en"
        )
        ...
        for role, key in (("Title", "headline"), ("Caption", "description_text")):
            ...
            elif key == "description_text" and picture.get("extra", {}).get(
                "event_description"
            ):
                # format caption using `event_description` field
                data_content = "Picture showing {}".format(
                    picture["extra"]["event_description"]
                )
            else:
            ...
            # string's length is used in original belga's newsml
            SubElement(characteristics, "SizeInBytes").text = (
                str(len(data_content)) if data_content else "0"
            )
            SubElement(
                characteristics,
                "Property",
    ...
    def _format_coverage(self, newscomponent_1_level, coverage):
        ...
        for role, key in (("Title", "headline"), ("Caption", "description_text")):
            ...
            # string's length is used in original belga's newsml
            SubElement(characteristics, "SizeInBytes").text = (
                str(len(coverage[key])) if coverage.get(key) else "0"
            )
            SubElement(
                characteristics,
                "Property",
    ...
    def _format_audio(self, newscomponent_1_level, audio):
        ...
        for role, key in (("Title", "headline"), ("Body", "description_text")):
            ...
            # string's length is used in original belga's newsml
            SubElement(characteristics, "SizeInBytes").text = (
                str(len(audio[key])) if audio.get(key) else "0"
            )
            SubElement(
                characteristics,
                "Property",
    ...
    def _format_video(self, newscomponent_1_level, video):
        ...
        for role, key in (("Title", "headline"), ("Body", "description_text")):
            ...
            elif key == "description_text" and video.get("extra", {}).get(
                "event_description"
            ):
                # format caption using `event_description` field
                data_content = "Video showing {}".format(
                    video["extra"]["event_description"]
                )
            else:
        ...
        self._format_media_contentitem(
            newscomponent_3_level, rendition=video["renditions"]["original"]
        )
    ...
    def _format_attachment(self, newscomponent_1_level, attachment):
        ...
        newscomponent_2_level = SubElement(
            newscomponent_1_level,
            "NewsComponent",
            {XML_LANG: attachment.get("language"), "Duid": str(attachment.get("_id"))},
        )
    ...
    def _set_belga_urn(self, media_item):
        ...
        for key, rendition in media_item.get("renditions", {}).items():
            ...
            # rendition is from Belga coverage search provider
                ...
                belga_id = media_item[GUID_FIELD].split(":")[-1]
                rendition["belga-urn"] = provider.GALLERY_URN.format(id=belga_id)
    ...
    def _format_media_contentitem(self, newscomponent_3_level, rendition):
        ...
        filename = (
            rendition["filename"]
            if rendition.get("filename")
            else rendition["href"].rsplit("/", 1)[-1]
        )
    ...
    def _format_newslines(self, newscomponent_2_level, item):
        ...
        for subject in item.get("subject", []):
            if subject.get("scheme") == "country":
                try:
                    SubElement(newslines, "KeywordLine").text = subject["translations"][
                        "name"
                    ][item.get("language")]
                except KeyError:
                    logger.warning(
        ...
        if item.get("extra", {}).get("belga-keywords"):
            for keyword in [
                i.strip() for i in item["extra"]["belga-keywords"].split(",")
            ]:
    ...
    def _format_administrative_metadata(self, newscomponent_2_level, item):
        ...
        administrative_metadata = SubElement(
            newscomponent_2_level, "AdministrativeMetadata"
        )
        SubElement(
        ...
        if item.get("type") == CONTENT_TYPE.PICTURE:
            authors = (
                (item["original_creator"],) if item.get("original_creator") else tuple()
            )
        else:
        ...
        if item.get("administrative", {}).get("contributor"):
            SubElement(
                SubElement(administrative_metadata, "Contributor"),
                "Party",
        ...
        if item.get("version_creator"):
            ...
            SubElement(
                ...
                {
                    "FormalName": "Validator",
                    "Value": item["administrative"]["validator"],
                },
            )
        SubElement(
            administrative_metadata,
            "Property",
            {
                "FormalName": "ValidationDate",
                "Value": self._get_formatted_datetime(item["firstpublished"]),
            },
        )
        if item.get("administrative", {}).get("foreign_id"):
            SubElement(
                ...
                "Property",
                {
                    "FormalName": "ForeignId",
                    "Value": item["administrative"]["foreign_id"],
                },
            )
        if item.get("urgency"):
            SubElement(
        ...
        for subject in item.get("subject", []):
            if subject.get("scheme") == "services-products":
                ...
                property_newspackage = SubElement(
                    administrative_metadata, "Property", {"FormalName": "NewsPackage"}
                )
                SubElement(
                    property_newspackage,
                    "Property",
            ...
            if subject.get("scheme") == "distribution":
                SubElement(
                    ...
                    "Property",
                    {
                        ...
                        "Value": get_distribution_value(subject["qcode"]),
                    },
                )
        ...
        sources = [
            ...
            if subj.get("scheme") == "sources"
        ]
        sources += [
            subj["qcode"]
            for subj in item.get("subject", [])
            if subj.get("scheme") == "media-source"
        ]
    ...
    def _format_descriptive_metadata(self, newscomponent_2_level, item):
        ...
        if item.get("firstcreated"):
            descriptive_metadata.attrib["DateAndTime"] = self._get_formatted_datetime(
                item["firstcreated"]
            )
        ...
        countries = [
            subj
            for subj in item.get("subject", [])
            if subj.get("scheme") == "countries"
        ]
        for country in countries:
            country_property = SubElement(
                location, "Property", {"FormalName": "Country"}
            )
            try:
                country_property.set(
                    "Value", country["translations"]["name"][item.get("language")]
                )
            except KeyError:
                logger.warning(
                    'There is no "{}" translation for countries cv. Country: {}'.format(
                        item.get("language"), country
                    )
                )
    ...
    def _format_newscomponent_3_level(self, newscomponent_2_level, item):
        ...
        for formalname, item_key in (
            ...
            if item.get(item_key):
                ...
                text = text_utils.get_text(
                    html_str, content="html", space_on_elements=True, space="   "
                )
                text = text.strip()
    ...
    def _get_newsml_items_chain(self, items_chain):
        ...
        sd_items_chain = deepcopy(
            tuple(
                i
                for i in items_chain
                if i.get(ITEM_STATE)
        ...
        for sd_item in sd_items_chain:
            ...
            media_items = [
                sd_item_associations[i]
                for i in sd_item_associations
                if sd_item_associations[i]
                and sd_item_associations[i].get(ITEM_TYPE)
                in (
                    CONTENT_TYPE.PICTURE,
                    CONTENT_TYPE.GRAPHIC,
                    CONTENT_TYPE.AUDIO,
            ...
            used_ids = []
            for picture in [
                i for i in media_items if i[ITEM_TYPE] == CONTENT_TYPE.PICTURE
            ]:
                ...
                newsml_item["language"] = sd_item.get(
                    "language", newsml_item.get("language")
                )
            ...
            used_ids = []
            for graphic in [
                i for i in media_items if i[ITEM_TYPE] == CONTENT_TYPE.GRAPHIC
            ]:
                ...
                if sd_item_extra.get(field_id):
                    for belga_item_id in sd_item_extra[field_id].split(";"):
                        ...
                        try:
                            ...
                            logger.warning(
                                "Failed to fetch belga coverage: {}".format(e)
                            )
                        else:
                            newsml_item = {
                                k: v for k, v in sd_item.items() if k in KEYS_TO_INHERIT
                            }
                            newsml_item.update(
                                belga_cov_search_provider.format_list_item(data)
                            )
                            newsml_item["guid"] = (
                                belga_cov_search_provider.GUID_PREFIX + gallery_id
                            )
                            newsml_item["_role"] = self.NEWSCOMPONENT2_ROLES.GALLERY
                            newsml_items_chain.append(newsml_item)
            # attachments
            ...
            rel_text_items = [
                ...
                if (
                    sd_item_associations[i]
                    and sd_item_associations[i].get(ITEM_TYPE) == "text"
