...
def set_default_metadata_with_translate(item, **kwargs):
    ...
    # SDBELGA-538
    filtered_subjects = [s for s in item["subject"] if s.get("scheme") == "services-products"]
    ...
    content_filter = get_resource_service("content_filters").find_one(req=None, _id=content_filter_id)
    ...
    if len(filtered_subjects) > 1 and content_filter:
        ...
        for filter in content_filter.get("content_filter", []):
            for filter_id in filter.get("expression", {}).get("fc", []):
                filter_condition = filter_conditions_service.find_one(req=None, _id=filter_id)
