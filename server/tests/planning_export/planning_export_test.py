...
class PlanningExportTests(TestCase):
    ...
    async def test_export_week(self):
        events = self.events_for_week
        dutch_template_data = await render_template(
        ...
        french_template_data = await render_template(
        ...
        french_template_headline_data = await render_template(
        ...
        dutch_template_headline_data = await render_template(
    ...
    async def test_export_tommorow(self):
        events = self.events_for_tommorow
        french_data = await render_template(
            "archived/french_news_events_tommorrow.html", items=events, app=self.app
        )
        dutch_data = await render_template(
            "archived/dutch_news_events_tommorrow.html", items=events, app=self.app
        )
        self.assertIn(
            (
    ...
    async def test_planning_advisory_export_tomorrow(self):
            ...
            planning_items.append(
                {
                    ...
                    "dates": ev["dates"],
                    "location": ev.get("location", {"name": "Unknown", "country": ""}),
                    "links": ev.get("links", []),
                    "coverages": [
                        {
    ...
    async def test_bilingual_planning_advisory_export_tomorrow(self):
        ...
        bilingual_data = await render_template(
    ...
    async def test_bilingual_events_advisory_export_tomorrow(self):
        ...
        bilingual_data = await render_template(
            "archived/bilingual_news_events_tommorrow.html",
        ...
        politics_section = bilingual_data.split("<h3>Politics</h3>")[1].split("<h3>")[0]
        self.assertNotIn(
            "00:00 - 23:59",
            politics_section,
    ...
    async def test_internal_bilingual_events_advisory_export_tomorrow(self):
        ...
        internal_data = await render_template(
            "archived/internal_bilingual_news_events_tomorrow.html",
            items=bilingual_events,
            app=self.app,
        )
    ...
    async def test_internal_bilingual_planning_advisory_export_tomorrow(self):
    ...
    async def test_bilingual_events_header_uses_earliest_date_no_leading_zero(self):
        """
        ...
        formatted = await format_event_for_tommorow_bilingual(events, locale="nl")
    ...
    async def test_internal_bilingual_events_header_uses_earliest_date_no_leading_zero(
        self,
    ):
        """
        ...
        formatted = await format_event_for_tommorow_bilingual_internal(
            events, locale="nl"
        )
    ...
    async def test_internal_bilingual_planning_uses_french_translation_and_header_date(
        self,
    ):
        """
        ...
        scheduled = datetime.datetime(2025, 11, 5, 10, 0, tzinfo=datetime.timezone.utc)
        ...
        formatted = await format_planning_for_tomorrow_bilingual_internal(
            [planning_item]
        )
    ...
    async def test_advisory_hides_all_day_no_time_and_shows_belga_prefix(self):
        ...
        timed_end = datetime.datetime(2025, 1, 10, 16, 0, tzinfo=datetime.timezone.utc)
        timed_event = {
            "_id": ObjectId(),
            "name": "Timed Briefing",
        ...
        events_html = await render_template(
            "archived/bilingual_news_events_tommorrow.html",
    ...
    async def test_bilingual_events_excludes_editorial_calendar(self):
    ...
    async def test_bilingual_planning_excludes_editorial_event_link(self):
    ...
    async def test_belga_image_photo_planning_renders_picture_only(self):
    ...
    async def test_belga_image_video_planning_renders_video_only(self):
    ...
    async def test_belga_image_video_planning_chronological_order(self):
    ...
    async def test_belga_image_photo_planning_event_ids_json(self):
        ...
        output = (
            await render_template(
                "archived/internal_image_photo_planning_event_ids.html",
                items=planning_items,
                app=self.app,
            )
        ).strip()
    ...
    async def test_belga_image_video_planning_event_ids_json(self):
        video_event_id = ObjectId()
        photo_event_id = ObjectId()
        ...
        output = (
            await render_template(
                "archived/internal_image_video_planning_event_ids.html",
                items=planning_items,
                app=self.app,
            )
        ).strip()
    ...
    async def test_internal_bilingual_planning_advisory_tomorrow_event_ids_json(self):
        event_id_1 = ObjectId()
        event_id_2 = ObjectId()
